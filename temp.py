...
for i in range(15):
    axs[i].imshow(X_batch[i].reshape(imgDims[0],imgDims[1]))
    axs[i].axis('off')
plt.show()
...
def myModel():
    ...
    model.add((Conv2D(no_Of_Filters,size_of_Filter,input_shape=(imgDims[0],imgDims[1],1),activation='relu')))  # ADDING MORE CONVOLUTION LAYERS = LESS FEATURES BUT CAN CAUSE ACCURACY TO INCREASE
